...
INSTALLER_REPO="${SNOWLUMA_INSTALLER_REPO:-fy0/sl-docker}"
...
IMAGE="${SNOWLUMA_IMAGE:-ghcr.io/fy0/sl-docker:latest}"
...
选项:
  ...
  --image REF        镜像（默认 ghcr.io/fy0/sl-docker:latest）
...
pull_image() {
  ...
  # Docker Hub 镜像加速镜像只适用于 docker.io 引用；ghcr 等其他 registry 直连
  case "${IMAGE}" in
    *.*/*|*:*/*) ;; # 带 registry 前缀, 跳过 docker hub 镜像
    *)
      refs+=(
